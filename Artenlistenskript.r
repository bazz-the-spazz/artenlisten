...
artenliste <- function(daten, kopf="kopf.md", titel=format(Sys.time(), "%b %Y"), fuss="dfhdfahadh", fontsize="\\large", table.length.adjust=0, wald=FALSE, waldextrablatt=T, Transekt, output="Artenliste.md", extrazeile, find.in.head, replace.in.head){
	require(openxlsx)
	if(missing(Transekt)) trans <- F else trans <- T
	if(!missing(kopf)){
	...
	for (i in 1:ncol(data)){ #loop for each column (=each Plot) of the data
		...
		if(trans){
			wald <- F
			sorto <- F
			if(!is.numeric(Transekt))  warning("Wieviele Aufnahmen im Transekt?") else{
				tmp <- 1:Transekt
				tmp[1] <- paste0(tmp[1], "(N)")
				tmp[length(tmp)] <- paste0(tmp[length(tmp)], "(S)")
				tmp <- paste0("& \\textbf{",tmp,"} ")
				tmp <- paste( "\\textbf{", names(data)[i] ,"} \\phantom{ChrysospleniumChrysospleniumChrysosplen}", paste(tmp, collapse = ""), "\\\\\\hline")
				
				header <- c(paste(c("\\begin{tabularx}{\\textwidth}{|l|", rep("X|", Transekt), "} \\hline"), collapse = ""), tmp) # this is the Transekt header
			}
		}
		...
		if(new.order){
			...
			if(sorto) arten <- sort(arten)          # sort
		...
		if(wald | trans) { # in forest we have to adjust the cutoff
			cutoff  <- cutoff -1
			cutoff2 <- cutoff2-1
		...
		if(trans){
			...
			restline <- paste(c(rep("& \\phantom{--}", Transekt-1), "& \\\\\\hline"), collapse = " ")
			emptyline <- paste("\\phantom{--}", restline)
			
			arten <- paste(arten, restline)
			
			if(length(arten) < cutoff) arten <- c(arten, rep(emptyline, cutoff-length(arten) ))  # FALLS Artenliste kürzer als erster Cuttoff, ergänzen mit leeren zeilen
			table <- c(header, arten[1:cutoff], end)  # Create first table
			...
			if(length(arten)> cutoff ){ # Create more tables when first page is full
				...
				if(length(arten) < cutoff2) arten <- c(arten, rep(emptyline, cutoff2-length(arten) ))  # FALLS Artenliste kürzer als erster Cuttoff, ergänzen mit leeren zeilen
				table <- c(table, "\\newpage", header, arten, end)
			}
			
			if( waldextrablatt &
					table[length(table)-8] != emptyline
			) {
				table <- c(table, "\\newpage", header, rep(emptyline, cutoff2), end)
				totpage <- totpage+1
			}
		}
...
eingabeformular <- function(daten, explo, kopf, wald=F, transekte,  filename = "eingabeformular.xlsx", dummy=FALSE, overwrite=FALSE, n.empty.rows=5){
	library(openxlsx)
	
	if(!missing(transekte)) transekt <- T else transekt <- F
	...
	if(wald){
		...
		if(transekt){
			l <- data.frame(lpl, l)
			for(i in 1:transekte) {
				l[,ncol(l)+1] <- ""
				l[which(l$l=="Transekt"), ncol(l)] <- paste0("T",i)
			}
		} else {
			l <- data.frame(lpl, l)
		}
	...
	for(i in 1:length(formating)) addStyle(wb, sheet = 1, cols=1:30, rows=formating[i], style = headStyle)
...
## Revised Function to order the species into grasses and rest. requires the new.order=TRUE in artenliste()
sort.as.funcgroup.data <- function(df, info, toad){
	info <- info[,c("In.Daten", "funcgroup")]
	rownames(info) <- info[,1]
	l <- list()
	m <- 0
	...
	for(i in 1:ncol(df)){
		...
		x <- x[x!=""]
		if(!missing(toad)) x <- unique(c(x,toad))
		test <- x %in% info$In.Daten
		if((length(unique(test))==1 & unique(test)[1]==TRUE)==FALSE) warn <- c(warn, x[!(x %in% info$In.Daten)])
		x <- info[x,]
		if("Gräser" %in% x$funcgroup){
			x <- c("Gräser", sort(x[x$funcgroup=="Gräser",1]), "", "", "Rest", sort(x[x$funcgroup!="Gräser",1]))
		} else {
	...
	if(length(warn)>0)	warning(.call=paste( c( sort(unique(warn)), " not in Infodata!"), collapse=" "))
