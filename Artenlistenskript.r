...
artenliste <- function(data, kopf="kopf.md", titel=format(Sys.time(), "%b %Y"), fuss="", fontsize="\\large", table.length.adjust=0){
  ...
  if(fuss=="") {fuss <- titel} else {paste(names(data)[i], fuss)}
	...
	## Generate the tables
		...
		if(length(arten)>cutoff & length(arten)<(cutoff*2)){ #wenn die Arten die zeilenanzahl von cutoff überschreiten-> 2 zeilen machen
			a1 <- arten[1:cutoff]
		...
		if(length(arten)>(cutoff*2)){   #wenn die Arten die zeilenanzahl von erster seite überschreiten-> mehrere Tabellen machen
			table <- character()
